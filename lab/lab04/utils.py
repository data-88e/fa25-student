...
def cobb_douglas_sol(A, K, L, alpha, beta = None):
    if beta is None:
        return A * K ** alpha * L ** (1 - alpha)
    else:
        return A * K ** alpha * L ** (beta)
        
def cobb_douglas_plotter_K_sol(A, L_bar, alpha, y):
    plt.plot(np.arange(0, 1.01, 0.01), y)
    plt.title(fr"Cobb-Douglas with $\bar L$ = {L_bar}, $A$ = {A} and $\alpha$ = {alpha}")
    plt.xlabel("Capital Stock")
    plt.ylabel("Output");
    
def cobb_douglas_plotter_L_sol(A, K_bar, alpha):
...
def plot_cobb_douglas_sol(V, orig_V):
...
def change_alpha_beta_sol(alpha_beta_sum):
    L_s = np.arange(0, 10.11, 0.1)
    K_s = np.arange(0, 10.11, 0.1)
    A = 1
